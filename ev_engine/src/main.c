...
// Track completed object names for room restore (g->player can complete in any order)
...
// Sprint 1: Clock deceleration g->state


// Sprint 2: Door positions for spatial g->audio (corridor)
...
static void show_text(GameCtx *g, const char *text) {
    g->vig_text = text;
    g->text_scale_target = 1.0f;
    g->text_y_offset = 20.0f;
    g->text_y_velocity = 0.0f;
}
static void hide_text(GameCtx *g) {
    g->text_scale_target = 0.0f;
}

// Rain drops for taxi g->scene
...
static void menu_open(GameCtx *g, MenuMode mode) { g->menu_mode = mode; g->menu_cursor = 0; menu_init_springs(g); g->menu_overlay_a = 0; g->menu_overlay_va = 0; EnableCursor(); }
static void menu_close(GameCtx *g) { g->menu_mode = MENU_NONE; g->menu_cursor = 0; g->menu_overlay_a = 0; g->menu_overlay_va = 0; if (g->state != STATE_TITLE) DisableCursor(); }

// Forward declaration
static void load_state(GameCtx *g, GameState s);

// Exposure helper — stores g->scene value and adds style bias
static void set_exposure(GameCtx *g, float exp) {
    g->scene_exposure = exp;
    SetPostFXExposure(&g->postfx, exp + visual_styles[g->current_style].exposure_bias);
}

static void update_menu_springs(GameCtx *g, float dt) {
...
static bool update_pause_menu(GameCtx *g) {
...
static void draw_pause_menu(GameCtx *g) {
...
static void transition_to(GameCtx *g, GameState s) {
...
static void hard_cut_to(GameCtx *g, GameState s) {
...
static void write_state_file(GameCtx *g, GameState s) {
    const char *names[] = {
...
static void load_state(GameCtx *g, GameState s) {
    ...
    // Reset PostFX to current visual style baseline before per-g->scene overrides
    ...
    switch (s) {
        case STATE_TITLE:
            ...
            reset_title_state();
            PlayTitleMusic(&g->audio);  // ambient atmosphere
        ...
        case STATE_CAR:
            ...
            StopAmbient(&g->audio);
            StopTitleMusic(&g->audio);  // fade out title atmosphere
            PlayCityAmbient(&g->audio);
            SetCityAmbientVolume(&g->audio, 0.04f);
            SetSceneLighting(&g->lighting, LightingPreset_Taxi());
            set_exposure(g, 0.05f);  // slight lift — see the interior
            SetPostFXGrain(&g->postfx, 0.45f);      // grainy night footage
            SetPostFXCA(&g->postfx, 1.5f);           // subtle CA — night feeling
            SetPostFXVignette(&g->postfx, 1.0f);     // intimate interior
            SetPostFXWarmth(&g->postfx, 0.1f);       // sodium warmth
            break;

        case STATE_DRIVING:
            // Taxi arriving — same g->scene, just stopped
            // Don't rebuild — keep the g->scene from STATE_CAR
            break;
        ...
        case STATE_HOTEL_EXT:
            ...
            set_exposure(g, 0.0f);   // let moonlight work
        ...
        case STATE_LOBBY:
            ...
            set_exposure(g, 0.0f);   // grand lobby — let chandelier work
        ...
        case STATE_ELEVATOR:
            ...
            if (g->elevator_to_corridor) {
                ...
                PlayDoorThud(&g->audio);  // doors closing behind you
                g->player.gravity_mult = 0.4f;  // orbital gravity — you're already in space
                SetSceneLighting(&g->lighting, LightingPreset_ElevatorSpace());
                set_exposure(g, -0.05f);  // slightly dimmer — between floors
                SetPostFXGrain(&g->postfx, 0.35f);
            } else {
                PlayElevatorWhoosh(&g->audio);  // ascending wind — building toward hyperspace
                g->player.gravity_mult = 1.0f;  // Earth gravity — last time you'll feel this
                SetSceneLighting(&g->lighting, LightingPreset_Elevator());
                set_exposure(g, 0.0f);
                SetPostFXGrain(&g->postfx, 0.3f);  // fluorescent — less grain
            }
            break;
        ...
        case STATE_HALLWAY:
            ...
            PlayMuffledPiano(&g->audio);    // someone else's room
            PlayFootstepsAbove(&g->audio);  // walking upstairs
            SetSceneLighting(&g->lighting, LightingPreset_Hallway());
            set_exposure(g, -0.05f);  // dim corridor — slight
        ...
        case STATE_ROOM: {
            ...
            PlayMuffledPiano(&g->audio);     // music from the next room
            PlayFootstepsAbove(&g->audio);   // someone pacing upstairs
            SetSceneLighting(&g->lighting, LightingPreset_Room());
            set_exposure(g, -0.05f);  // slight darkness — 2AM
            SetPostFXGrain(&g->postfx, 0.5f);
            break;
        }
        ...
        case STATE_BATHROOM:
            ...
            set_exposure(g, 0.1f);   // bright bathroom fluorescent
            SetPostFXGrain(&g->postfx, 0.3f);      // clinical — less grain
            break;
        ...
        case STATE_BALCONY:
            ...
            StopClockAmbient(&g->audio);
            StopCityAmbient(&g->audio);
            StopSuiteMusic(&g->audio);  // lighthouse ends
            PlayBalconyMusic(&g->audio);  // ambient4 — the void outside
            PlayWindAmbient(&g->audio);  // void wind — no city sounds in orbit
            PlayBalconyGust(&g->audio);  // one-shot rush — the void entering
            g->player.gravity_mult = 0.3f;  // exposed to void — lightest gravity
            SetPostFXWarmth(&g->postfx, 1.0f);
            SetSceneLighting(&g->lighting, LightingPreset_Balcony());
            set_exposure(g, 0.05f);  // slight lift — Earth glow
            SetPostFXGrain(&g->postfx, 0.6f);      // contemplative grain
            break;

        case STATE_BED:
            StopAmbient(&g->audio); StopCityAmbient(&g->audio); StopWindAmbient(&g->audio);
            StopSuiteMusic(&g->audio); StopBalconyMusic(&g->audio);
            PlayBedImpact(&g->audio);  // soft muffled thud — surrender
        ...
        case STATE_STARS: {
            ...
            PlayHeldChord(&g->audio);  // stacked fifths — credits as elegy
        ...
        case STATE_PARIS_DREAM:
            build_hotel_room(&g->scene);
            init_player(&g->player, g->scene.spawn);
            g->player.control_mult = 0.5f;  // dream movement — slow, deliberate
            g->player.gravity_mult = 0.7f;  // dream gravity — slightly floaty
            ...
            StopAmbient(&g->audio);
            StopClockAmbient(&g->audio);  // NO clock in the dream
            StopWindAmbient(&g->audio);
            StartAmbient(&g->audio, DRONE_ROOM);
            SetSceneLighting(&g->lighting, LightingPreset_ParisDream());
            set_exposure(g, 0.1f);                     // bright golden
            SetPostFXWarmth(&g->postfx, 0.7f);         // warm but not total wash
            SetPostFXGrain(&g->postfx, 0.7f);          // dreamy grain
            SetPostFXSaturation(&g->postfx, 1.2f);     // oversaturated
            SetPostFXCA(&g->postfx, 3.5f);             // dream shimmer
            SetPostFXVignette(&g->postfx, 2.0f);       // heavy vignette — darkens edges for contrast
            SetPostFXContrast(&g->postfx, 1.5f);       // actually punchy — drives shadows darker
            g->scene.fog_color = (Color){30, 22, 10, 255};  // darker golden fog
            g->scene.fog_density = 0.005f;              // thinner — let shadows breathe
            break;
        ...
        case STATE_RETURN_TAXI:
            ...
            StopWindAmbient(&g->audio);
            PlayCityAmbient(&g->audio);
            SetCityAmbientVolume(&g->audio, 0.02f);  // quiet dawn
            SetSceneLighting(&g->lighting, LightingPreset_ReturnTaxi());
            set_exposure(g, 0.05f);                    // slight lift
            SetPostFXWarmth(&g->postfx, 0.4f);         // warm but gentle
            SetPostFXGrain(&g->postfx, 0.3f);          // clean dawn
            SetPostFXSaturation(&g->postfx, 1.05f);    // natural
            SetPostFXCA(&g->postfx, 1.5f);             // minimal
            SetPostFXVignette(&g->postfx, 1.0f);       // default
            SetPostFXContrast(&g->postfx, 1.0f);       // default
            break;
        ...
        case STATE_HYPERSPACE:
            ...
            PlayHyperspaceTone(&g->audio);  // rising 80Hz→400Hz glissando
            PlayHyperspaceRiser(&g->audio); // fat layered riser — noise+harmonics+sub
            SetSceneLighting(&g->lighting, LightingPreset_Hyperspace());
            set_exposure(g, 0.1f);
            SetPostFXGrain(&g->postfx, 0.7f);
            SetPostFXCA(&g->postfx, 5.0f);  // start with elevated CA
            break;

        case STATE_SPACE_LOBBY:
            build_space_lobby(&g->scene);
            init_player(&g->player, g->scene.spawn);
            g->elevator_ding_played = false;  // reset for arrival ding
            StopAmbient(&g->audio);
            StopClockAmbient(&g->audio);
            StopCityAmbient(&g->audio);
            StopWindAmbient(&g->audio);
            ...
            PlayArrivalThump(&g->audio);    // deep bass impact — you have landed
            PlayGravitySettle(&g->audio);   // hull creak — the ship acknowledges your weight
            PlayAirlockHiss(&g->audio);     // pressurization — you're aboard
            PlayEarthPresence(&g->audio);   // 30Hz sub-bass — the planet's weight
            g->player.gravity_mult = 0.4f;  // orbital gravity — jumps soar, wall runs linger
            StartAmbient(&g->audio, DRONE_SPACE_LOBBY);
            SetSceneLighting(&g->lighting, LightingPreset_SpaceLobby());
            set_exposure(g, -0.05f);
            SetPostFXGrain(&g->postfx, 0.4f);
            SetPostFXCA(&g->postfx, 2.5f);  // reset CA from hyperspace
            ...
            if (g->lobby_visit_count > 1) {
                ...
                set_exposure(g, -0.05f + revisit_warmth * 0.1f);
            }
        ...
        case STATE_SPACE_CORRIDOR:
            ...
            StopCityAmbient(&g->audio);
            StopWindAmbient(&g->audio);
            PlayAirlockHiss(&g->audio);  // pressurization — entering sealed corridor
            PlayDoorThud(&g->audio);     // bulkhead sealing behind you
            g->player.gravity_mult = 0.4f;  // orbital gravity persists
            StartAmbient(&g->audio, DRONE_SPACE_CORRIDOR);
            PlayCorridorMusic(&g->audio);  // ambient1 — underneath the walk
            ...
            g->door_positions[0] = (Vector3){-3.5f, 1.6f, 4.0f};   // warm light door
            g->door_positions[1] = (Vector3){3.5f, 1.6f, 8.0f};    // blue light door
            g->door_positions[2] = (Vector3){-3.5f, 1.6f, 12.0f};  // dark door — silence
        ...
        case STATE_SPACE_SUITE:
            ...
            PlayDoorThud(&g->audio);     // suite door closing — sealed in
            PlayAirlockHiss(&g->audio);  // pressurization equalize
            g->player.gravity_mult = 0.5f;  // suite has more gravity — domestic, grounded
            StartAmbient(&g->audio, DRONE_SPACE_SUITE);
            PlayClockAmbient(&g->audio);  // the room's heartbeat
            g->audio.clock_rate = 1.0f;   // reset clock rate
            SetSoundPitch(g->audio.snd_clock, 1.0f);
            g->agency_removal_timer = 0;
            SetSceneLighting(&g->lighting, LightingPreset_SpaceSuite());
            set_exposure(g, -0.1f);
            SetPostFXGrain(&g->postfx, 0.35f);
            // Gibbons — walks to sofa, sits, waits for g->player to finish tasks
    ...
    printf("[DBG] load_state(g, %d): walls=%d objs=%d exposure=%.2f style=%d shadow=%s fog=%.4f\n",
...
static void draw_vignette_text(GameCtx *g) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyDown(KEY_LEFT_SHIFT) || IsKeyDown(KEY_RIGHT_SHIFT)) {
            ...
            if (new_style >= 0 && new_style < STYLE_COUNT) {
                ...
                SetPostFXExposure(&g->postfx, g->scene_exposure + visual_styles[g->current_style].exposure_bias);
                show_text(g, visual_styles[g->current_style].name);
            }
        } else {
        // Dev g->scene jumps — number keys for instant room testing (no shift held)
        if (IsKeyPressed(KEY_ONE))   load_state(g, STATE_HOTEL_EXT);
        if (IsKeyPressed(KEY_TWO))   load_state(g, STATE_LOBBY);
        if (IsKeyPressed(KEY_THREE)) load_state(g, STATE_HALLWAY);
        if (IsKeyPressed(KEY_FOUR))  load_state(g, STATE_ROOM);
        if (IsKeyPressed(KEY_FIVE))  load_state(g, STATE_BALCONY);
        if (IsKeyPressed(KEY_SIX))   load_state(g, STATE_BATHROOM);
        if (IsKeyPressed(KEY_SEVEN)) load_state(g, STATE_SPACE_LOBBY);
        if (IsKeyPressed(KEY_EIGHT)) load_state(g, STATE_SPACE_CORRIDOR);
        if (IsKeyPressed(KEY_NINE))  load_state(g, STATE_SPACE_SUITE);
        if (IsKeyPressed(KEY_ZERO))  load_state(g, STATE_CAR);
        }
#endif // PLAYTEST
        ...
        switch (g->state) {
            case STATE_TITLE: {
                ...
                if (g->state_time > 1.0f && !title_breath_played) {
                    PlayTitleBreath(&g->audio);  // subliminal inhale before text
                    title_breath_played = true;
                }
                if (IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_SPACE)) {
                    title_breath_played = false;
                    transition_to(g, STATE_CAR);
                }
                break;
            }
            ...
            case STATE_CAR: {
                ...
                // Driver speaks — two lines, the frame story
                {
                    ...
                    if (g->state_time > 1.5f && !driver_line1) {
                        show_text(g, "Where to?");
                        driver_line1 = true;
                    }
                    if (g->state_time > 4.0f && !driver_line2) {
                        show_text(g, "Sky Tower. Three hours.");
                        driver_line2 = true;
                    }
                }
                ...
                if (g->state_time > 10.0f) {
                    float ramp = (g->state_time - 10.0f) / 3.5f;  // 0→1 over last 3.5s
                ...
                if (g->state_time > 1.5f && g->state_time < 2.0f) show_text(g, "Auckland. 2:47 AM.");
                if (g->state_time > 4.0f && g->state_time < 4.5f) hide_text(g);
                if (g->state_time > 5.5f && g->state_time < 6.0f) show_text(g, "You going up to the tower?");
                if (g->state_time > 8.0f && g->state_time < 8.5f) hide_text(g);
                if (g->state_time > 9.0f && g->state_time < 9.5f) show_text(g, "They say there's a hotel up there now.");
                if (g->state_time > 12.0f && g->state_time < 12.5f) hide_text(g);
                if (g->state_time > 12.5f && g->state_time < 13.0f) show_text(g, "Three hours to kill.");
                ...
                if (g->state_time > 13.5f) {
                    ...
                    hard_cut_to(g, STATE_HYPERSPACE);
                }
            ...
            case STATE_DRIVING: {
                // Arrival — taxi stopped, same g->scene, mouse look only
                ...
                if (g->state_time > 7.0f) transition_to(g, STATE_HOTEL_EXT);
                if (IsKeyPressed(KEY_ENTER) && g->state_time > 2) transition_to(g, STATE_HOTEL_EXT);
                break;
            }
            ...
            case STATE_HOTEL_EXT:
                ...
                if (g->scene.has_exit) {
                    ...
                    // Threshold crossing — slow the g->player as they approach the door
                    if (dist < 3.0f) {
                    ...
                    if (dist < 1.5f) transition_to(g, STATE_LOBBY);
                }
                break;
                        ...
                        if (dist < obj->radius) {
                            ...
                            if (Vector3DotProduct(to, look) > 0.5f) {
                                if (strcmp(obj->name, "elevator") == 0) {
                                    ...
                                    transition_to(g, STATE_ELEVATOR);
                                    break;
                                }
                                if (strcmp(obj->name, "newspaper") == 0) {
                                    obj->step++; obj->done = true;
                                    PlayInteract(&g->audio, INTERACT_FABRIC);
                                    show_text(g, "ORBITAL SUITE OPENS — THREE HOURS TO KILL");
                                    break;
                                }
                                if (strcmp(obj->name, "bell") == 0) {
                                    obj->step++; obj->done = true;
                                    PlayElevatorDing(&g->audio);  // same ding — foreshadowing
            ...
            case STATE_ELEVATOR:
                if (g->elevator_to_corridor) {
                    ...
                    if (g->state_time > 4.2f) {
                        ...
                        hard_cut_to(g, STATE_SPACE_CORRIDOR);
                    }
                } else {
                    ...
                    // The threshold. Everything you know drops away below.
                    {
                        ...
                        if (t > 3.0f) {
                            ...
                            g->player.camera.fovy = 70.0f - narrow * 15.0f;  // 70→55
                        }
                    }
            ...
            case STATE_HYPERSPACE:
                ...
                // Phase 4 (4.5-6s): release, FOV snaps to 90, hard cut
                {
                    ...
                    if (t < 4.0f) {
                        ...
                        g->player.camera.position.z -= speed * dt;
                        g->player.camera.target.z -= speed * dt;

                        if (t < 2.0f) {
                            // Phase 1: gentle accel, FOV widens
                            g->player.camera.fovy = 70.0f + t * 7.5f;  // 70→85
                        } else {
                            // Phase 2: tunnel narrows, FOV compresses
                            float p2 = (t - 2.0f) / 2.0f;  // 0→1 over 2s
                            g->player.camera.fovy = 85.0f - p2 * 25.0f;  // 85→60
                ...
                if (g->state_time > 6.0f) {
                    ...
                    SetMasterVolume(0.0f);
                    hard_cut_to(g, STATE_SPACE_LOBBY);
                }
                break;
            ...
            case STATE_HALLWAY:
                ...
                if (g->scene.has_exit) {
                    float dist = Vector3Distance(g->player.camera.position, g->scene.exit_pos);
                    if (dist < 1.5f) transition_to(g, STATE_ROOM);
                }
                break;
                        ...
                        if (dist < obj->radius) {
                            ...
                            if (Vector3DotProduct(to, look) > 0.5f) {
                                ...
                                if (strcmp(obj->name, "bathroom") == 0) {
                                    transition_to(g, STATE_BATHROOM);
                                    break;
                                }
                                ...
                                obj->step++;
                                PlayInteract(&g->audio, get_interact_sound(obj->name));
                                kick_camera(&g->player, -0.01f, 0.005f);
                                ...
                                if (obj->step >= obj->max_steps) {
                                    ...
                                    set_exposure(g, -0.05f + progress * 0.15f);
                                    ...
                                    if (g->tasks_done >= PARIS_TASK_COUNT) {
                                        ...
                                        g->scene.fog_density = 0.001f;
                                        SetPostFXWarmth(&g->postfx, 1.0f);
                                        set_exposure(g, 0.1f);  // fully settled — bright
                                    } else {
            ...
            case STATE_BALCONY:
                ...
                if (g->cigarette_anim) {
                    ...
                    // Disable g->player movement during animation
                ...
                if (g->state_time > 14.0f)
                    hard_cut_to(g, STATE_PARIS_DREAM);
            ...
            case STATE_BED:
                ...
                // Progressive desaturation — color drains as consciousness fades
                {
                    ...
                    SetPostFXSaturation(&g->postfx, 0.92f - desat * 0.6f);  // 0.92 → 0.32
                ...
                // Breathing — camera rises and falls like lungs
                {
                    ...
                    g->player.camera.position.y += breath * dt * 2.0f;
                }
                break;
            ...
            case STATE_STARS:
                ...
                {
                    float drift_yaw = g->state_time * 0.003f;  // ~0.17°/sec
            ...
            case STATE_RETURN_TAXI: {
                ...
                if (g->state_time > 3.0f && g->state_time < 3.5f) show_text(g, "Auckland. 5:52 AM.");
                if (g->state_time > 6.0f && g->state_time < 6.5f) hide_text(g);
                if (g->state_time > 8.0f && g->state_time < 8.5f) show_text(g, "Good hotel?");
                if (g->state_time > 11.0f && g->state_time < 11.5f) hide_text(g);
            ...
            case STATE_SPACE_LOBBY:
                ...
                // Elevator shaft is at (0, y, 0) — center of lobby
                {
                    ...
                    if (elev_dist < 1.5f && !g->gibbons.active) {
                        // Gibbons finished and g->player is at elevator — ride up
                        show_text(g, "Going up.");
            ...
            case STATE_SPACE_CORRIDOR:
                ...
                if (g->gibbons.waiting) {
                    if (g->gibbons.current_waypoint == 1) {
                        ...
                        g->gibbons.yaw_target = 1.5708f;  // face the window (90°)
                ...
                // The g->player feels the corridor "pulling" them toward their room.
                if (g->scene.has_exit && g->player.vel.z > 0.1f) {
                    g->player.vel.z *= 1.03f;  // 3% — subliminal
                }
                // ============================================================
                ...
                // ============================================================
                {
                    ...
                    for (int di = 0; di < 2; di++) {
                        ...
                        // Spatial panning — left/right based on g->player facing
                    ...
                    if (dark_dist < 4.0f) {
                        ...
                        set_exposure(g, 0.0f - dark_t * 0.15f);
                    }
                }
                if (g->scene.has_exit) {
                    float dist = Vector3Distance(g->player.camera.position, g->scene.exit_pos);
                    if (dist < 3.0f) {
                        ...
                        transition_to(g, STATE_SPACE_SUITE);
                    }
                }
            ...
            case STATE_SPACE_SUITE:
                ...
                if (!g->gibbons.active && g->gibbons.current_line >= g->gibbons.line_count) {
                    ...
                    g->gibbons.pos.y = -100;  // move off-screen so shadow doesn't linger
                }
                ...
                if (g->interaction_phases[1] == 1) {
                    ...
                    if (g->interaction_timers[1] <= 0) {
                        ...
                        add_wall(&g->scene, -3.1f, 0.41f, 3.5f, 0.05f, 0.06f, 0.05f,
                                 (Color){240,210,100,200});
                        add_sphere(&g->scene, -2.8f, 0.7f, 3.3f, 0.1f,
                                   (Color){240,210,100,180});
                        add_sphere(&g->scene, -3.2f, 0.9f, 3.6f, 0.08f,
                                   (Color){240,210,100,160});
                        add_sphere(&g->scene, -3.0f, 1.2f, 3.4f, 0.06f,
                ...
                if (g->interaction_phases[0] == 2) {
                    ...
                    SetPointLightIdx(&g->lighting, 0, -2.5f, 1.2f, -4.8f,
                        ...
                        if (dist < obj->radius) {
                            ...
                            if (Vector3DotProduct(to, look) > 0.5f) {
                                ...
                                if (strcmp(obj->name, "lamp") == 0 && obj->step == 1) {
                                    ...
                                    add_wall(&g->scene, -2.5f, 1.0f, -4.65f, 0.15f, 0.25f, 0.15f,
                                    ...
                                    add_wall(&g->scene, 5.3f, 0.45f, -1.6f, 0.4f, 0.04f, 0.3f,
                                             (Color){55,85,175,255});
                                    ...
                                    add_wall(&g->scene, 0, 0.54f, -4.3f, 2.8f, 0.02f, 1.4f,
                                    ...
                                    add_cone(&g->scene, -3.1f, 0.39f, 3.5f, 0.06f, 0.08f,
                                             (Color){210,210,215,200});
                                    add_cylinder(&g->scene, -3.1f, 0.44f, 3.5f, 0.02f, 0.08f,
                                                 (Color){210,210,215,200});
                                }
                                ...
                                if (obj->step >= obj->max_steps) {
                                    ...
                                    if (strcmp(obj->name, "lamp") == 0) {
                                        ...
                                        add_light_panel(&g->scene, -2.5f, 1.2f, -4.8f,
                                                        0.5f, 0.6f, 0.5f, (Color){255,220,120,200});
                                        // Lamp casts warm shadow on wall behind
                                        add_wall(&g->scene, -2.5f, 1.8f, -5.35f,
                                                 1.2f, 1.5f, 0.01f, (Color){60,45,20,40});
                                    } else if (strcmp(obj->name, "desk") == 0) {
                                        // Star chart spread on desk
                                        add_wall(&g->scene, 5.5f, 0.85f, -2, 1.8f, 0.01f, 0.8f,
                                                 (Color){20,25,45,255});
                                        // Star dots — constellations
                                        add_wall(&g->scene, 5.2f, 0.86f, -2.1f, 0.06f, 0.005f, 0.06f,
                                                 (Color){240,235,220,200});
                                        add_wall(&g->scene, 5.7f, 0.86f, -1.8f, 0.04f, 0.005f, 0.04f,
                                                 (Color){240,235,220,180});
                                        add_wall(&g->scene, 5.4f, 0.86f, -1.6f, 0.05f, 0.005f, 0.05f,
                                                 (Color){240,235,220,160});
                                        add_wall(&g->scene, 6.0f, 0.86f, -2.3f, 0.03f, 0.005f, 0.03f,
                                                 (Color){240,235,220,170});
                                        // Route line — pencil mark between stars
                                        add_wall(&g->scene, 5.45f, 0.862f, -1.95f, 0.8f, 0.003f, 0.02f,
                                                 (Color){180,60,50,100});
                                        // Annotation circle — red pencil around destination
                                        add_cylinder(&g->scene, 5.7f, 0.863f, -1.8f, 0.12f, 0.003f,
                                                     (Color){180,60,50,80});
                                    } else if (strcmp(obj->name, "bed") == 0) {
                                        // P3: Start bed descent ritual
                                        add_wall(&g->scene, 0, 0.68f, -5.0f, 0.18f, 0.04f, 0.12f,
                                                 (Color){90,50,25,255});
                                        ...
                                        add_sphere(&g->scene, -2.9f, 2.0f, 3.5f, 0.04f,
                                                   (Color){180,200,240,100});
                                        // Second escapee — smaller, higher, almost gone
                                        add_sphere(&g->scene, -3.1f, 2.8f, 3.35f, 0.025f,
                ...
                if (g->tasks_done >= SPACE_TASK_COUNT) {
                    ...
                    if (g->done_pause < 4.0f) {
                        float removal = g->done_pause / 4.0f;  // 0→1
                        g->player.control_mult = 1.0f - removal;
                    } else {
                        g->player.control_mult = 0.0f;
                    }
                    if (g->done_pause > 5.0f) {
                        g->player.control_mult = 1.0f;  // restore for balcony
                        g->balcony_flash_triggered = false;
                        g->balcony_flash_timer = 0;
                        hard_cut_to(g, STATE_BALCONY);
                    }
        ...
        if (g->menu_mode != MENU_NONE) draw_pause_menu(g);
