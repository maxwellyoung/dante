...
static const char *fs_source =
    ...
    "\n"
    "    // Key light — hard Lambert, real shadows (Skin Deep contrast)\n"
    "    float NdotL = dot(norm, -lightDir);\n"
    "    float lambert = max(NdotL, 0.0);\n"
    "    // Slight wrap to avoid pure black (0.05 floor)\n"
    "    lambert = lambert * 0.95 + 0.05;\n"
    "    vec3 keyDiffuse = lightColor * lambert * (1.0 - shadow * 0.7);\n"
    "\n"
    "    // Fill light — soft counter (no wrap, just clamped)\n"
    "    float NdotF = dot(norm, -fillDir);\n"
    "    float fillLambert = max(NdotF, 0.0);\n"
    ...
    "\n"
    "    // Normal-based AO — gentle darkening for downward-facing surfaces\n"
