...
static const char *fs_source =
    ...
    "        float n = fbm(surfUV * 1.5);\n"
    "        baseColor *= 0.96 + n * 0.08;\n"
    "        specMult = 0.08;\n"
    "        bumpHeight = n * 0.07;\n"
    "    }\n"
    "    else if (materialId == 1) {\n"
    "        // MARBLE — veined with Voronoi cellular structure\n"
    "        float vein = sin(surfUV.x * 2.0 + surfUV.y * 1.0 + sin(wp.z * 1.5) * 2.0);\n"
    "        vein = smoothstep(0.0, 0.15, abs(vein));\n"
    "        float n = fbm(surfUV * 2.5);\n"
    "        float cell = voronoi(surfUV * 1.5);\n"
    "        float vein2 = smoothstep(0.02, 0.08, cell) * 0.15;\n"
    "        baseColor *= mix(0.90, 1.0, vein) + n * 0.03 + vein2;\n"
    "        specMult = 0.35;\n"
    "        // Polished marble: gentle bumps from veins\n"
    "        bumpHeight = (1.0 - vein) * 0.05 + n * 0.03;\n"
    "    }\n"
    "    else if (materialId == 2) {\n"
    "        // WOOD — grain lines along one axis\n"
    "        float grain = sin(surfUV.y * 6.0 + noise(surfUV * 2.0) * 3.0) * 0.5 + 0.5;\n"
    "        grain = smoothstep(0.3, 0.7, grain);\n"
    "        float ring = noise(vec2(surfUV.x * 0.75, surfUV.y * 4.0));\n"
    "        baseColor *= 0.93 + grain * 0.06 + ring * 0.03;\n"
    "        specMult = 0.12;\n"
    "        bumpHeight = grain * 0.09 + ring * 0.035;\n"
    "    }\n"
    "    else if (materialId == 3) {\n"
    "        // CARPET — dense fuzzy noise, matte\n"
    "        float n = fbm(surfUV * 4.0);\n"
    "        baseColor *= 0.94 + n * 0.09;\n"
    "        specMult = 0.0;\n"
    "        bumpHeight = n * 0.042;\n"  // carpet pile
    "    }\n"
    "    else if (materialId == 4) {\n"
    "        // WALLPAPER — vertical stripe with subtle noise\n"
    "        float stripe = sin(surfUV.x * 4.0) * 0.5 + 0.5;\n"
    "        stripe = smoothstep(0.3, 0.7, stripe);\n"
    "        float n = noise(surfUV * 1.5);\n"
    "        baseColor *= 0.97 + stripe * 0.025 + n * 0.015;\n"
    "        specMult = 0.05;\n"
    "        bumpHeight = stripe * 0.02;\n"
    "    }\n"
    "    else if (materialId == 5) {\n"
    "        // TILE — grid with grout lines\n"
    "        vec2 tileUV = fract(surfUV * 1.5);\n"
    "        float grout = step(0.06, tileUV.x) * step(0.06, tileUV.y);\n"
    "        baseColor *= mix(0.82, 1.0, grout);\n"
    "        specMult = 0.25;\n"
    "        bumpHeight = grout * 0.12;\n"  // recessed grout lines
    "    }\n"
    "    else if (materialId == 6) {\n"
    "        // BRASS — metallic shimmer with subtle patina tarnish\n"
    "        float n = noise(surfUV * 10.0);\n"
    "        float patina = voronoi(surfUV * 4.0);\n"
    "        float tarnish = smoothstep(0.0, 0.3, patina) * 0.12;\n"
    "        baseColor *= 0.96 + n * 0.04 + tarnish;\n"
    "        baseColor.g += (1.0 - patina) * 0.015;\n"
    "        specMult = 0.55 + patina * 0.15;\n"
    "        bumpHeight = n * 0.05 + (1.0 - patina) * 0.035;\n"  // tooling marks + patina texture
    "    }\n"
    "    else if (materialId == 7) {\n"
    "        // GLASS — subtle surface distortion\n"
    "        float n = noise(surfUV * 15.0);\n"
    "        baseColor *= 0.99 + n * 0.02;\n"
    "        specMult = 0.5;\n"
    ...
    "        float grain = noise(surfUV * 12.5);\n"
    "        baseColor *= 0.95 + n * 0.04 + grain * 0.02;\n"
    "        specMult = 0.18;\n"
    "        bumpHeight = n * 0.077 + grain * 0.042;\n"  // leather grain catches light
    "    }\n"
    "    else if (materialId == 9) {\n"
    "        // FABRIC — soft weave pattern\n"
    "        vec2 weave = fract(surfUV * 3.0);\n"
    "        float warp = step(0.5, weave.x);\n"
    "        float weft = step(0.5, weave.y);\n"
    "        float pattern = abs(warp - weft) * 0.08;\n"
    "        float n = noise(surfUV * 6.0);\n"
    "        baseColor *= 0.96 + pattern + n * 0.03;\n"
    "        specMult = 0.02;\n"
    "        bumpHeight = pattern * 0.5 + n * 0.035;\n"  // weave texture
    "    }\n"
    "    else if (materialId == 10) {\n"
    "        // CHECKERBOARD — two-tone floor\n"
    "        vec2 checker = floor(surfUV * 1.0);\n"
    "        float check = mod(checker.x + checker.y, 2.0);\n"
    "        baseColor *= mix(0.85, 1.0, check);\n"
    "        float tileNoise = noise(checker * 3.65) * 0.04;\n"
    "        baseColor += tileNoise;\n"
    "        specMult = 0.3;\n"
    "        bumpHeight = tileNoise * 0.8;\n"
    "    }\n"
    ...
    "        float grain = sin(grainDir * 10.0 + noise(surfUV * 2.5) * 3.0) * 0.5 + 0.5;\n"
    "        grain = smoothstep(0.3, 0.7, grain);\n"
    "        baseColor *= mix(0.91, 1.0, plank) + grain * 0.03;\n"
    "        vec2 plankUV = fract(vec2(shifted, uv.y));\n"
    "        float grout = step(0.04, plankUV.x) * step(0.04, plankUV.y);\n"
    "        baseColor *= mix(0.85, 1.0, grout);\n"
    "        specMult = 0.15;\n"
    "        bumpHeight = grain * 0.07 + (1.0 - grout) * 0.105;\n"  // plank edges + grain
    "    }\n"
    "    else if (materialId == 12) {\n"
    "        // PARQUET — alternating grain direction in tiles\n"
    "        vec2 tile = floor(surfUV * 1.25);\n"
    "        float dir = mod(tile.x + tile.y, 2.0);\n"
    "        vec2 localUV = fract(surfUV * 1.25);\n"
    "        float grainCoord = (dir > 0.5) ? localUV.x : localUV.y;\n"
    "        float grain = sin(grainCoord * 12.5 + noise(surfUV * 3.0) * 2.0) * 0.5 + 0.5;\n"
    "        baseColor *= 0.95 + grain * 0.05;\n"
    "        float border = step(0.03, localUV.x) * step(0.03, localUV.y)\n"
    "                      * step(0.03, 1.0-localUV.x) * step(0.03, 1.0-localUV.y);\n"
    "        baseColor *= mix(0.87, 1.0, border);\n"
    "        specMult = 0.14;\n"
    "        bumpHeight = grain * 0.063 + (1.0 - border) * 0.088;\n"
    "    }\n"
    ...
    "        float nap = noise(surfUV * 7.5) * 0.06;\n"
    "        baseColor *= 0.94 + sheen + nap;\n"
    "        specMult = 0.03;\n"
    "        bumpHeight = nap * 0.6;\n"  // velvet nap
    "    }\n"
    "    else if (materialId == 14) {\n"
    "        // EMISSIVE — light panels, screens, fixtures\n"
    "        float n = noise(surfUV * 5.0);\n"
    "        baseColor *= 1.0 + n * 0.025;\n"
    "        specMult = 0.0;\n"
    "        isEmissive = 1.0;\n"
    "    }\n"
    "\n"
