...
local function build_animation(sw, sh, animation, default_render)
    local frames = {}
...
function PlayerSpriteBank.load(path)
    ...
    for _, name in ipairs(bank.animation_order) do
        bank.animations[name] =
            build_animation(sw, sh, manifest.animations[name] or {}, bank.render)
    end

    for name, animation in pairs(manifest.animations or {}) do
        if bank.animations[name] == nil then
            bank.animations[name] = build_animation(sw, sh, animation, bank.render)
        end
    end
